...
class TestTxOutputSerialization(unittest.TestCase):
    ...
    def test_serialize_01(self):
        ...
        self.assertTrue(len(split_items) == 6)
        self.assertEqual(split_items[0], TRANSACTION_01_HASH)
        self.assertEqual(split_items[1], str(TRANSACTION_01_INDEX))
        self.assertEqual(split_items[2], str(TRANSACTION_01_BLOCK_HASH))
        self.assertEqual(split_items[3], str(TRANSACTION_01_VALUE))
        self.assertEqual(split_items[4], TRANSACTION_01_SCRIPT)
        self.assertEqual(split_items[5], TRANSACTION_01_TYPE)

    def test_deserialize_01(self):
        serialized_data = "5a189242e85c9670cefac381de8423c11fd9d4b0ebcf86468282e0fc1fe78fb8,0,00000000000271a2dc26e7667f8419f2e15416dc6955e5a6c6cdf3f2574dd08e,1240000000,2103bddd330f9f666ba93f46e6dd2717aba0878e1ecefbe5860373b2524f064a13f5ac,pubkey"
        ...
        self.assertEqual(txoutput_01.block_hash, txout.block_hash)
...
class TXOutputFromDictionaryAndGets(unittest.TestCase):
    ...
    def test_get_pubkey_real_tx(self):
        ...
        output = TXOutput.from_dictionary(pubkey_txid, TRANSACTION_02_BLOCK_HASH, transaction["vout"][0])
    ...
    def test_get_pubkey_hash_real_tx(self):
        ...
        output = TXOutput.from_dictionary(pubkey_hash_txid, TRANSACTION_02_BLOCK_HASH, transaction["vout"][0],)
        ...
        output = TXOutput.from_dictionary(pubkey_hash_txid, TRANSACTION_02_BLOCK_HASH, transaction["vout"][1])
        self.assertEqual(output.script_type, "pubkeyhash")
        self.assertIsNone(output.get_pubkey())
